...
func (s *SessionService) handleAccountMethod(ctx context.Context, request device.Request) (any, error) {
	...
	case "accounts.create":
		...
		return map[string]any{"account": accountView(createdAccount), "profile": profileView(createdProfile, nil)}, nil
	...
	case "accounts.list":
		...
		page, err := s.Store.ListAccounts(ctx, storage.AccountListOptions{Provider: body.Provider, Limit: body.Limit, Cursor: body.Cursor})
		if err != nil {
			return nil, err
		}
	...
	case "accounts.show":
		...
		for _, profile := range profiles.Items {
			profileViews = append(profileViews, profileView(profile, nil))
		}
		return map[string]any{"account": accountView(account), "profiles": profileViews, "next_cursor": nullableString(profiles.NextCursor)}, nil
	...
	case "accounts.update", "accounts.disable", "accounts.enable", "accounts.delete":
		...
		case "accounts.disable", "accounts.enable":
			updated, err := s.Store.SetAccountEnabled(ctx, account.ID, body.ExpectedRevision, request.Method == "accounts.enable", s.now())
			if err != nil {
				return nil, err
			}
			return accountView(updated), nil
	...
	case "profiles.create":
		...
		return profileView(profile, nil), nil
	...
	case "profiles.list":
		...
		for _, profile := range page.Items {
			items = append(items, profileView(profile, nil))
		}
		return map[string]any{"items": items, "next_cursor": nullableString(page.NextCursor)}, nil
	...
	case "profiles.update", "profiles.disable", "profiles.enable", "profiles.delete":
		...
		case "profiles.update":
			...
			return profileView(profile, binding.Credential), nil
	...
	case "usage.list":
		...
		snapshots, err := s.Store.ListUsageSnapshots(ctx, accountID)
		if err != nil {
			return nil, err
		}
		items := make([]any, 0, len(snapshots))
		for _, snapshot := range snapshots {
			items = append(items, usageView(snapshot, s.now()))
		}
...
func profileView(profile domain.RuntimeProfile, credential *domain.CredentialInstance) map[string]any {
...
func bindingView(binding storage.ProfileBinding) map[string]any {
	return map[string]any{"account": accountView(binding.Account), "profile": profileView(binding.Profile, binding.Credential)}
}

func usageView(snapshot domain.UsageSnapshot, now time.Time) map[string]any {
