...
- Resume 不是恢复旧记录：当 Capability 包含 `session.resume` 时，创建新的 Session，并用 `resumedFromSessionId` 和 `providerSessionId` 关联旧会话。
...
- Daemon 启动 app-server 子进程，优先使用 stdio；Unix 可选 Unix Socket。
- 不将 app-server 实验性 WebSocket 直接暴露到网络。
- Daemon 将 Codex JSON-RPC 转换成 MultiAgentDesk 稳定事件。
- 每个 RuntimeProfile 使用独立 `CODEX_HOME`。
- 登录使用官方交互式登录或 device auth。
...
Phase 2 冻结前必须完成 Codex Spike：

- 对当前 app-server schema 验证 Usage/Rate Limit 方法和字段；不存在时将相关能力降级而不是伪造官方数据。
- 在 macOS 验证 file credential store 与 `CODEX_HOME` 的实际落盘行为。
- 在无浏览器 Linux 验证 device auth。
- 在两台隔离设备并发使用同一受管凭据至少 48 小时并触发刷新，确认 refresh token 的轮换/吊销语义。
- 至少用连续三个受支持 Codex CLI 版本运行 fixture replay；不通过的版本在 Compatibility Matrix 中标为不支持。
...
- `claude auth status` JSON 用于登录健康检查。
- 用户可在来源设备执行 `claude setup-token`，通过 E2EE 授权给服务器。
- 目标 Daemon 通过 `CLAUDE_CODE_OAUTH_TOKEN` 注入受管 Token。
- 不读取或复制 macOS Keychain 数据库。
- 交互会话运行于跨平台 PTY；Windows 使用 ConPTY。
...
1. Phase 0.5 先验证两个 `CLAUDE_CONFIG_DIR` 的 Keychain 条目和真实请求身份能否隔离。
2. 若能够隔离，则使用独立 Config Dir 的交互登录。
3. 若 Keychain 为全局共享，则受管 Profile 必须改用用户生成的 `setup-token`，通过 `CLAUDE_CODE_OAUTH_TOKEN` 注入；不得声称 Config Dir 已隔离登录态。
4. 只有在 Linux 交互 PTY、长会话和吊销测试全部通过后，setup-token 路径才可进入稳定能力矩阵。
5. `claude auth status` JSON 是首选健康检查；若目标版本不存在或结构不兼容，只能降级为 best-effort 探测并明确标记来源。
...
- 无系统密钥环的 Headless Linux：用户设置 Vault 密码，经 Argon2id 派生 KEK。

Vault 有明确的 `locked | unlocked` 状态：
...
- SQLite 只保存 AEAD 密文、nonce、版本和关联元数据。
- 每个 CredentialInstance 使用独立随机 DEK。
- DEK 由设备 Vault 主密钥包装。
- 日志、错误和审计事件禁止包含秘密值。
...
- Windows：ConPTY 全屏 TUI、Tauri Sidecar、Named Pipe 最小原型。
...
- 独立 `CODEX_HOME` 和经验证的登录/物化路径。
- app-server client、事件映射、Approval 和已验证的 Usage 能力。
- Codex fixture、record/replay、版本门禁和 Compatibility Matrix。

出口：在 Linux 服务器使用指定 Codex Account 创建真实 Session，并从第二个本地 CLI 获取 ControllerLease、处理审批和恢复 Provider 会话。

### Phase 3：Claude Vertical Slice

阻塞依赖：Phase 0.5 的 Keychain 隔离、auth status、setup-token 交互 PTY 和 ConPTY 结论。

交付：

- 按 Spike 决策使用独立 `CLAUDE_CONFIG_DIR` 或 setup-token 注入。
