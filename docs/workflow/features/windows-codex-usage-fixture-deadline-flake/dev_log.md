...
- The `MaxWait` race blocker is repaired and scoped checks are green. The
  exact repaired SHA is unpublished, so it has no native Windows CI evidence;
  cross-compilation cannot clear that evidence gate.
