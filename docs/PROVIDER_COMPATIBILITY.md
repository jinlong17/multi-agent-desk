...
| Claude distinct-account subscription integration | CLI `2.1.207` macOS; `2.1.132` Linux; official policy reviewed 2026-07-16 | macOS 26.5.2 arm64 + Linux 5.4.0 x86_64 | documented side-by-side `CLAUDE_CONFIG_DIR`; optional official 5h/7d status-line schema; third-party product authentication and billing policy | `spikes/claude-distinct-accounts/2026-07-16-policy-and-isolation-spike.md`, sanitized JSON sibling | Unsupported as a stable managed subscription surface absent explicit Anthropic approval; products/tools for others are directed to API-key or supported-cloud auth; distinct technical identities and Usage binding not run after the decisive policy failure | direct official Claude Code outside the managed surface, or user-supplied Claude Console API key/supported cloud provider with explicit billing source; no subscription credential routing or quota dashboard | 2026-07-16 |
