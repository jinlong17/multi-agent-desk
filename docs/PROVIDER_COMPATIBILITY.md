...
| Codex distinct-account managed Homes | CLI `0.144.2` | Linux 5.4 x86_64 live | two official interactive identities in isolated `0600` Homes; distinct concurrent Provider sessions; concurrent account-bound Usage; active-logout denial; target-scoped stop/logout/re-login without cross-account mutation | `spikes/codex-distinct-accounts/2026-07-16-distinct-account-homes-spike.md`, sanitized JSON sibling; `reviews/spike-codex-distinct-account-homes/2026-07-16-security-review.md` | Security-reviewed support for exact Linux `0.144.2`; stable alias launch still requires privacy-preserving post-login identity binding/confirmation; macOS distinct-identity, real Windows, and passive-soak claims remain open | one explicitly selected target-local managed Home; official interactive login; no auto-rotation; unknown identity/version/revision ambiguity fails closed | 2026-07-16 |
| Codex explicit multi-account selector | CLI `0.144.2` | Linux amd64 live; macOS arm64 schema/empty-home evidence only; Windows amd64 compile/protocol only | daemon-issued preview, typed `@alias` confirmation, immutable reserved Session, distinct A/B app-server/Usage, active-logout denial, target-only re-login, zero retained materialized auth files | `workflow/features/codex-multi-account-selector/p2-as-built.md`, `reviews/codex-multi-account-selector/2026-07-20-feature-verify.md` | Exact Linux amd64 `0.144.2` P2 verified; macOS returns `schema_compatible_identity_acceptance_pending`; Windows and other architectures return `provider_platform_unsupported` before materialization/spawn | use one explicitly selected target-local Profile and official login on accepted Linux; no raw-ID bypass, default Account, auto-rotation, or cross-platform fallback | 2026-07-20 |
...
| Claude distinct-account subscription integration | CLI `2.1.207` macOS; `2.1.132` Linux; official policy reviewed 2026-07-16 | macOS 26.5.2 arm64 + Linux 5.4.0 x86_64 | documented side-by-side `CLAUDE_CONFIG_DIR`; optional official 5h/7d status-line schema; third-party product authentication and billing policy | `spikes/claude-distinct-accounts/2026-07-16-policy-and-isolation-spike.md`, sanitized JSON sibling; `reviews/spike-claude-distinct-account-usage/2026-07-16-security-review.md` | Security-reviewed as unsupported for a stable managed subscription surface absent explicit Anthropic approval; products/tools for others are directed to API-key or supported-cloud auth; distinct technical identities and Usage binding were not run after the decisive policy failure | direct official Claude Code outside the managed surface, or separately planned user-supplied Claude Console API key/supported cloud provider with explicit billing source; no subscription credential routing or quota dashboard | 2026-07-16 |
| Claude Team-subscription direct CLI/PTY compatibility | CLI `2.1.207` | macOS 26.5.2 arm64 | one one-shot no-tool direct official-CLI print process plus fail-closed PTY gate under existing `claude.ai` Team auth, with no API key/cloud override or raw-content evidence | `spikes/claude-subscription/2026-07-20-macos-team-cli-pty-spike.md`, sanitized JSON sibling, attempt ledger, and `reviews/spike-claude-subscription-cli-pty-compatibility/2026-07-20-security-review.md` | Security-reviewed negative evidence for this exact version/platform: print returned the fixed marker with exit `0`, but the frozen aggregate selected-config metadata no-write criterion failed; PTY was not run. This is not positive CLI/PTY compatibility and establishes no stable managed subscription, Usage, billing, account, or Phase 3 capability; Provider HTTP-request count was not observed | direct official Claude Code outside the managed surface; any cache-write allowlist or PTY experiment requires a new feature-plan scope, fresh one-shot ledger, and Security Gate | 2026-07-20 |
...
The explicit multi-account selector applies a narrower gate than general
app-server schema discovery. Only Linux `amd64` reaches selector preview/runtime
materialization. macOS schema compatibility is not identity acceptance and
returns `schema_compatible_identity_acceptance_pending`; Windows returns
`provider_platform_unsupported`. Both outcomes occur before a selector-owned
Provider process or credential Home is created.
