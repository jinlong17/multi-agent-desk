...
| Claude Console API-key CLI/PTY | CLI `2.1.207` no-key arm; valid-key/Linux versions pending | macOS 26.5.2 arm64 no-key only; Linux amd64 pending | exact binary/help/auth-source presence; isolated invalid-key fail-closed and persistence sanitizer; valid billing/PTY/status projection not run | `spikes/claude-api-key/2026-07-20-compatibility-spike.md`, sanitized JSON sibling | Pending and blocked; no stable support claim until a dedicated key, exact Linux target, explicitly authorized bounded paid requests, two-platform PTY evidence and Security Review resolve the gate | no managed Claude Session; use direct official Claude Code or resume the reviewed Spike without subscription/cloud fallback | 2026-07-20 |
